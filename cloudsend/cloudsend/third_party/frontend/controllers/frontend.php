...
class Frontend extends CWX_Controller {
    ...
    public function centerlogin( $errortype = false, $errormsg = '' ) {
    ...
    public function centerverify() {
	$this->load->helper( array('form') );
	$this->load->library('form_validation');
	
	$config = array(
	    array( 'field' => 'inputEmail',	'label' => __('front_lbl_email'),	'rules' => 'trim|required|valid_email' ),
	...
	if( $this->form_validation->run() == false ) {
	    ...
	    $this->mGlobal->log( array( 'type' => 'error', 'message' => "Someone {$_email}tried to center log in.", 'size' => NULL ) );
	    $errormsg = validation_errors( ' ', '<br />' );
	    $this->centerlogin( 'error', $errormsg );
	} else {
	    $_account = $this->mFrontend->validateCenterLogin();
	    	    
	    if( $_account != false ) {	
		
		switch( $_account->level ):
		    case '1': // superadmin
		    case '2': // admin
			$_session = array(
			...
			$_session = array(
			    'frontlogin' => true,
			    'frontuser' => $_account->userURL,
			    'userReceiveNot' => $_account->userReceiveNot,
			    'userUnique' => $_account->userUniqueID,
			    'userid' => $_account->userUniqueID
			);
			$_redirect = $_account->userURL.'/index';
			break;
		endswitch;
		...
		$_email = ( ( isset( $_email ) && !empty( $_email ) ) ? 'with email "'.$_email.'" ' : '' );
		$this->mGlobal->log( array( 'type' => 'error', 'message' => "Someone {$_email}tried to center log in.", 'size' => NULL ) );		
		$this->center( 'error', __('account_msg_loginerror') );
	    }
	}
