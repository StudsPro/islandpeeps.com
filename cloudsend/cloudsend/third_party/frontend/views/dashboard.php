 ...
 */
  ...
  if( (int)$user->userMaxFileSize > 0 ) {
      $_max_size = roundsize( $user->userMaxFileSize );
  } else {
      $_max_size = roundsize( convertBytes( ini_get('upload_max_filesize') ) );
  }
...
<div class="container">
	...
	<div class="span<?php echo $_span ?>">
		...
		<h3><?php echo __('front_head_dashboard') ?></h3>
	    </div>
	    <blockquote>
		<p><?php echo __('front_desc_dashboard', array( $_max_size ) ) ?></p>
	    </blockquote>
	...
	<div class="row fileupload-buttonbar">
		...
		<span class="dsbl btn fileinput-button">
		    <i class="icon-plus"></i>
		    <span><?php echo __('front_btn_choosefile') ?></span>
		    <input type="file" name="files[]" multiple>
		</span>
		<button type="submit" class="dsbl btn start">
		    <i class="icon-upload"></i>
		    <span><?php echo __('front_btn_startupload') ?></span>
		</button>
		<button type="reset" class="btn cancel">
		    <i class="icon-ban-circle"></i>
		    <span><?php echo __('front_btn_cancelupload') ?></span>
		</button>
		<button type="button" class="btn restart hidden" onClick="location.href='<?php echo site_url( $user->userURL ) ?>';">
...
$(document).ready(function() {
	...
	.bind('fileuploadstop', function( e ) {
	    ...
	    $.post('<?php echo site_url( $user->userURL.'/finished' ) ?>', { 'data': fileResult } );
	    $('#fileupload').fileupload({
		dropZone: false
	    });
        })
