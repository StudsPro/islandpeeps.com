 ...
 if(affilateright("masterlist","write")==true)
   ...
   public function changestatus() 
    ...
    public function resizeimage()
    {
      ...
      // print_r($results);  
       foreach($results as $result)
       {
         if(!empty($result["image"]))
         {
            ...
            {
                  echo $result["image"];
            echo  $imageurl=SITE_UPLOADPATH.$result["image"];
            echo "<br>";
                    $imageurl=SITE_UPLOADPATH.$result["image"];
                    $key=$result["id"];
				    //********************************* Resize ****************************
                                            ...
                                            $config = array( 
                                                    'image_library' => 'gd2',
                                                    'source_image' => $imageurl,
                                                    'new_image' => SITE_UPLOADPATH."newimage/",
                                                    'maintain_ratio' => false,
                                                    'overwrite' => TRUE,
      ...
      echo "image resizes"; exit;
    }
  
  function addslug()
  {
     $sql="SELECT tbl_profiles.* FROM tbl_profiles
         ";
       $sql.=" where 1=1  order by id "; 
    ...
    foreach($result as $results)
    {
        ...
        $title=str_replace("-"," ",$results["title"]);
        $slug=url_title($title,"-",true); 
        $id=$results["id"];
        $this->db->query("update tbl_profiles set slug='".$slug."' where id='".$id."'");
    } 
