   ...
   function index($catid="")
      ...
      // ca61ac2f09986aeb4867f90cda0d3720
        /***************************************/
         ...
         {
            $profilebithdays =$this->db->query("SELECT count(id) as count FROM `tbl_profiles` WHERE year(dob)='".$currentyear."' and month(dob)='".$montid."' ORDER BY `id` ASC  ");
            foreach($profilebithdays->result() as $profilebithday)
           {
              $data['profilebob'].= '{  label: "'.$monthname.'['.$profilebithday->count.']",data: '.$profilebithday->count.' },';
           }
         }
	   ...
	   $pro_byadmin_arr=$profiles_by_admin->result();
        $data['profilebyadmin'].= '{  label: "'.$admindetails->username.'['.$pro_byadmin_arr[0]->count.']",data: '.$pro_byadmin_arr[0]->count.' },';
       }
