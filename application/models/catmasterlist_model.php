<?php
class Catmasterlist_model  extends CI_Model 
{
 ...
 //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX Customer Module Start
  public function GetAllrecords($input)
	{
      $str ='';
     // echo $input['fillter']; exit;
     if(!empty($input['fillter']))
     {
        $input['fillter']=str_replace("%20"," ",$input['fillter']);
	  if($input['fillter']<>'title' && $input['fillter']<>'undefined' )
	  $str = " and tbl_profiles.category='".$input['fillter']."'";
     }
   ...
   public function Getkinds($input)
   {
     ...
     public function Getsuggestionkinds()
   {
         
      $sql = "SELECT count(*) as pcount,kind FROM tbl_suggestion where 1=1  GROUP BY kind ORDER BY  `kind` DESC ";
	    $query=$this->db->query($sql);
        return $query->result_array();
 ...
 //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX Customer Module End 
  ...
  public function setbackgroundimage($id)
                      	...
                      	if (!$this->upload->do_upload('images'))
							{    $error = $this->upload->display_errors();   }
                               ...
                               $image_data = $this->upload->data();
                                ...
                                $peopleprofile_image=$uploadConst['file_name'];
   ...
   public function categoryinsert($input)
   {
    ...
    public function check_namecaetgory($name)
	{
    	$query = $this->db->get_where('tbl_category', array('id' => $name));
