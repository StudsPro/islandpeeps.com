...
if(trim($input['value']) !=''){
   $strSql1 = "SELECT title,image,description,region_id,id,slug FROM `tbl_profiles` WHERE (`title` LIKE '%".stripslashes($input['value'])."%' OR `tags` LIKE '%".stripslashes($input['value'])."%' OR `category` LIKE '%".stripslashes($input['value'])."%' OR `kind` LIKE '%".stripslashes($input['value'])."%') AND status='4'";
...
foreach($objRecord->result() as $profile)
{
    ...
    $regionName = $regionName_arr[0]->region_name;
   $re_selector=$this->removeUnsed($regionName)."_".$profile->id;
	if($profile->title !=''){
		//$data.=' <li onclick="showprofile(\''."#".$this->removeUnsed($regionName)."-profile".'\')"><a data-postid="'. $this->removeUnsed($regionName).'" href="#'.$this->removeUnsed($regionName).'-profile" data-title="'.utf8_encode(stripslashes($profile->title)).'" data-body="'.utf8_encode(stripslashes($profile->description)).'" data-image="'.base_url().'library/upload/'.$profile->image.'" title="'. utf8_encode(stripslashes($profile->title)).'"  class="post-thumb-video '. $this->removeUnsed($regionName).' imageclick recimg"><img src="'.base_url().'library/upload/'.$profile->image.'" width="50" height="50" />&nbsp;'.utf8_encode(stripslashes($profile->title)).'</a></li>';
	$data.=' <li onclick="showprofile(\''."".$regionName_arr[0]->id."".'\',\''."".$this->removeUnsed($regionName)."".'\',\''."".$re_selector."".'\',\''."".$profile->id."".'\')"><a id="getsearchprofile_'.$profile->id.'" data-slug="'.$profile->slug.'"  data-proid="'.$profile->id.'"  data-postid="'. $this->removeUnsed($regionName).'" href="javascript:void(0)" data-proid"'.$profile->id.'" data-postid"'.$this->removeUnsed($regionName).'"   data-regionid="'.$regionName_arr[0]->id.'"  data-title="'.utf8_encode(stripslashes($profile->title)).'" data-body="'.utf8_encode(stripslashes($profile->description)).'" data-image="'.base_url().'library/upload/'.$profile->image.'" title="'. utf8_encode(stripslashes($profile->title)).'"  class="post-thumb-video "><img src="'.base_url().'library/upload/'.$profile->image.'" width="50" height="50" />&nbsp;'.utf8_encode(stripslashes($profile->title)).'</a></li>';
	...
	if(trim($input['value']) !=''){
	 	$strSql1 = "SELECT id,title,image,description,region_id,slug FROM `tbl_profiles` WHERE (`title` LIKE '%".stripslashes($input['value'])."%' OR `tags` LIKE '%".stripslashes($input['value'])."%' OR `category` LIKE '%".stripslashes($input['value'])."%' OR `kind` LIKE '%".stripslashes($input['value'])."%') AND status='4'";
	    $objRecord = $this->db->query($strSql1);
		$rcnt =array();
		if($objRecord->num_rows > 0){
			$data ='';
		...
		foreach($objRecord->result() as $profile)
        {
				...
				if($profile->title !=''){
					...
					foreach($objRs->result() as $objRow)	
					{
						...
						if (array_key_exists($regionNameArr,$countryArr))
				  		{
				  			$countryArr[$regionNameArr][] = array('id'=>$profile->id,'title'=>$profile->title,'image'=>$profile->image,'description'=>$profile->description,'region_id'=>$objRow->id,'slug'=>$profile->slug);
				  		}
				   		else
				  		{
				  			$countryArr[$regionNameArr][] = array('id'=>$profile->id,'title'=>$profile->title,'image'=>$profile->image,'description'=>$profile->description,'region_id'=>$objRow->id,'slug'=>$profile->slug);
			...
			foreach($countryArr as $Ckey=>$Cval)
			{
				...
				foreach($Cval as $dkey=>$dval)
				{
					...
					$counrtyDataList.= '<a data-postid="'.$this->removeUnsed($Ckey).'" data-slug="'.$dval['slug'].'"  data-proid="'.$dval['id'].'" data-regionid="'.$dval['region_id'].'" href="javascript: void(0);" data-title="'.stripslashes(html_entity_decode($dval['title'], ENT_QUOTES)).'" data-body="'.stripslashes(htmlentities($dval['description'], ENT_QUOTES)).'" data-image="'.base_url().'library/upload/'.$dval['image'].'" title="'.stripslashes(html_entity_decode($dval['title'], ENT_QUOTES)).'" class="span_3 post-thumb-video '.$this->removeUnsed($Ckey).'_'.$dval['id'].' searchResultClick" data-selector="'.$this->removeUnsed($Ckey).'_'.$dval['id'].'"><img class="lazy" src="'.base_url().'library/upload/'.$dval['image'].'" width="80" height="100"/>'.stripslashes($dval['title']).'</a>';
				}
			$counrtyDataList.= '</div>';
			$counter++;
			}
