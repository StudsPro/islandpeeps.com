...
class Ads_model  extends CI_Model 
{
 ...
 public function GetadsById($id)
	{
	   $query = $this->db->query("SELECT * FROM tbl_ads where id=".$id);
	   return $query->result_array();
	}
...
private function mpeg2Mp4($video_file, $convertName)
 ...
 //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX Customer Module End 
  ...
  public function check_name($name)
	{
    	$query = $this->db->get_where('tbl_ads', array('title' => $name));
     ...
     public function insert($input)
    {
              ...
              if(!empty($input["regions"]))
              {
                 ...
                 if($input["ads_image"])
                  {   
                     $getimage=pathinfo($input["ads_image"]);
                     
                        
                  $arrvideo_value=array("mp4","flv","bat"); 
                             if(in_array($getimage['extension'],$arrvideo_value))
                             { 
