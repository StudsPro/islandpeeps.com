...
<style>
.funfacts {color: #ce675b;font-size: 2em !important;font-weight: normal !important;}
.getmargin{ margin-top: 20%;}
.search-ac{z-index: 9999999 !important;}
.prodesc{font-size: 20px;}
</style>
...
<!--<img src="</?php //echo base_url();?>show-thumbpj.php?src=</?php echo  base_url()."library/upload/".$banners->image;?>" width="200" style="margin-right:20px;" border="0" align="left" class="lazy"> -->
<?php } ?>
...
if(($rowAbout->id<>4) && ($rowAbout->id<>6)){?>
...
<li class="menu-tab"> <a role="tab" class="about menu-link">Suggestion</a> </li>
...
<h2 class="heading"><?php echo $arrProfiles->kind.$regionName;?></h2>
<?php
...
var disqus_shortname = 'islandpeeps2014'; //
...
feeds: {
	...
	instagram: {id: '<?php echo $instagram->instagramid;?>',accessToken: '<?php echo $instagram->accessToken;?>',redirectUrl: 'http://localhost/island',clientId: '<?php echo $instagram->clientId;?>',comments: '<?php echo $instagram->comments;?>',likes: '<?php echo $instagram->like;?>'},
	},
...
function showregionForSearch(regionid,selector){
var rid = $(regionid).attr('data-id');
var rcnt = $(regionid).attr('data-cnt');
$( regionid ).load( "<?php echo base_url();?>home/getregions?id="+rid+"&sno="+rcnt,function(responseTxt,statusTxt,xhr){
...
$('.statsLink').click(function(){$('html,body').animate({scrollTop: $('#screen-8').offset().top},'fast'); 
...
if(loadid != 'anguilla'){
showregionForSearch('#'+loadid,'.'+selector);
...
var url ='<img src="'+imgurl+'" border="0" class="lazy">';
$("#"+postid+"-player").html(url);
$('html,body').animate({scrollTop: $('#'+postid+'-player').offset().top},'slow');
/* FACEBOOK START */
var fbimgname = imgurl.substring(imgurl.lastIndexOf("upload/")+7,imgurl.lastIndexOf("&w"));
var fbimag = '<?php echo SITE_GETUPLOADPATH;?>'+fbimgname;
var fburl = '<?php echo BASE_URL;?>#'+postid;
