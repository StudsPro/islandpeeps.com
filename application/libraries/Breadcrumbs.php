	...
	function show()
	{
		if ($this->breadcrumbs) {
			...
			foreach ($this->breadcrumbs as $key => $crumb) {
				...
				if (end($keys) == $key) {
					$output .= $this->crumb_last_open . '' . html_entity_decode($crumb['page']) . '' . $this->crumb_close;
				} else {
					$output .= $this->crumb_open.'<a href="' . $crumb['href'] . '">' . html_entity_decode($crumb['page']) . '</a> '.$this->crumb_divider.$this->crumb_close;
				}
			}
