...
$search = array($lastyear,$currentyear,"01","02","03","04","05","06","07","08","09","10","11","12");
$replace = array("","","January","February","March","April","May","June","July","August","September","October","November","December");
$search2 = array ("<");
$replace2 = array ("");
$startyear=(date("Y")-1);
$startmonth=strlen((date("m")+1))==1 ?'0'.(date("m")+1) : (date("m")+1);
$startday=date("d");

 $startdate=''.$startyear.'-'.$startmonth.'-'.$startday.'';  
$ga->requestReportData(ga_profile_id,array('month','year'),array('pageviews','visits','bounces','entrances'),array('-year','-month') ,$filter=null,$start_date=$startdate,$end_date=$today,$start_index=1,$max_results=1000);
$yearlypageviews="";

foreach($ga->getResults() as $result){
/*
 '<tr valign="center" align="center">
<td width="25%">'. str_replace($search,$replace,$result).'</td>
<td width="20%">'.$result->getYear(str_replace($search2,$replace2,$result)).'</td> 
<td width="30%">'.$result->getPageviews().'</td>
<td width="25%">'.$result->getVisits().'</td>
</tr>';
*/
  $yearlypageviews .= '{  label: "'.str_replace($search,$replace,$result).'-'.$result->getYear(str_replace($search2,$replace2,$result)).' ['.$result->getPageviews().']",data: '.$result->getPageviews().' },';
}


 //********************************  Month *****************
//$start= date('Y-m-01', strtotime('-1 month')); //first day in last month
//$end= date('Y-m-t', strtotime('-1 month')); //last day in last month
...
$sort_metric1 = array('month');
$ga->requestReportData(ga_profile_id, 'month', array('pageviews', 'visits'),null, null , $start, $end, null, null);
foreach ($ga->getResults() as $dados) {
   // echo 'Total visits in last month: ' . $dados->getVisits();
   // echo '<br>';
   // echo 'Total pageviews in last month: ' . $dados->getPageviews();
    
     $monthlypageviews .= '{  label: "'.date('M Y', strtotime($start)).' ['.$dados->getPageviews().']",data: '.$dados->getPageviews().' },'; 
}
//echo $monthlypageviews; echo "<br>";
 //exit;
 //*********************  Days ***********************
...
$filter = null;
$Date=date("Y-m-d");
$start_date = date('Y-m-d', strtotime($Date. '-10 days'));
$end_date = date("Y-m-d");
$start_index = 1;
$max_results = 10000;
...
foreach($ga->getResults() as $result)
{
  ...
  $daypageviews.= '{  label: "'.date('jS M', strtotime($result->getDate())).' ['.$result->getPageviews().']",data: '.$result->getPageviews().' },';   
...
foreach($ga->getResults() as $result)
{
  ...
  $countries .= '{  label: "'.$result->getCountry().' ['.$result->getVisits().']",data: '.$result->getVisits().' },';
    ...
    $visitspercountry.= '{  country: "'.$result->getCountry().'",visits: '.$result->getVisits().',color: "'.$color[$i].'" },';
  $i++;
}
...
$monthvisits ='';
$monthpageviews = '';
$month_pagetrackingviews = '';
$mi=0;
foreach($ga->getResults() as $result)
{
 //echo '<pre>';print_r($result);echo '</pre>';
 
 $month_pagetrackingviews.= '{  label: "'.date('jS M', strtotime($result->getYear().'-'.$result->getMonth().'-'.$result->getDay())).'", value1: '.$result->getVisits().',color: "'.$color[$mi].'" },';
 
$monthvisits .='[gd('.$result->getYear().', '.$result->getMonth().', '.$result->getDay().') , '.$result->getVisits().'] ,'; 
$monthpageviews .='[gd('.$result->getYear().', '.$result->getMonth().', '.$result->getDay().') , '.$result->getPageviews().'] ,'; 
 $mi++;
}
 //echo $month_pagetrackingviews; exit;
//**********************************************************************************************
...
$userType.= '{  label: "'.$result->getUserType().' ['.$result->getVisits().']",data: '.$result->getVisits().' },';
 $sut++;
}
